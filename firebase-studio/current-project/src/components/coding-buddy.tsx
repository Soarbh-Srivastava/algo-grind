...
export function CodingBuddy() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Prepare history: take only the last 2 messages (if available)
      // These represent the last user interaction and the last AI response.
      const historyForFlow = chatHistory.slice(-2);

      const input: ChatInput = { 
        message: newMessage.content, // The current user message
        history: historyForFlow, // The limited, preceding conversation history
