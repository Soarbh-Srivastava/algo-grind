...
// Firebase configuration directly from process.env
...
if (!getApps().length) {
  const requiredKeys: (keyof typeof firebaseConfig)[] = [
    ...
    // measurementId is often optional. If it's not strictly required for your Firebase setup to initialize auth/firestore,
    // you can comment it out from this requiredKeys array. However, if your app relies on analytics, it might be needed.
    // 'measurementId' 
  ];

  const missingOrInvalidKeysDetails = requiredKeys
    .map(key => {
      // Correctly form the expected environment variable name (e.g., apiKey -> NEXT_PUBLIC_FIREBASE_API_KEY)
      const envVarName = `NEXT_PUBLIC_FIREBASE_${key.replace(/([A-Z])/g, '_$1').toUpperCase().replace(/^_/, '')}`;
      ...
      return { key, envVarName, valueProvided: String(value), isInvalid };
    })
  ...
  if (missingOrInvalidKeysDetails.length > 0) {
    const errorDetailsString = missingOrInvalidKeysDetails
      .map(detail => `- ${detail.envVarName}: (Value as seen by app: '${detail.valueProvided}') - Expected a valid string.`)
      .join('\n    ');
    ...
    Action Required:
    1. Please ensure all NEXT_PUBLIC_FIREBASE_... environment variables are correctly set in your .env file (located at the root of your project, e.g., /firebase-studio/current-project/.env) or your project's hosting/build configuration.
  ...
  try {
    ...
    console.error("Firebase Config Used (API Key Redacted for security if preferred, showing raw for debug):", JSON.stringify(firebaseConfig, (k, v) => k === "apiKey" && String(v).length > 5 ? "REDACTED_FOR_LOGGING_IF_NEEDED" : v, 2));
...
try {
  ...
  console.error("Firebase Config Used at time of error (API Key potentially redacted):", JSON.stringify(firebaseConfig, (k, v) => k === "apiKey" && String(v).length > 5 ? "REDACTED_FOR_LOGGING_IF_NEEDED" : v, 2));
